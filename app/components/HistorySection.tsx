...
return (
  <div className='mt-2'>
    <div className='pl-7 pt-5 bg-[#202127] h-100 ml-3 w-306 rounded-lg'>
      {/* Tab Buttons */}
      <div className='flex space-x-4'> {/* Added flex and space-x for better button layout */}
        <button
          className={`cursor-pointer px-4 py-2 ${activeTab === 'openOrders' ? 'text-white border-b-2 border-blue-500' : 'text-stone-300 hover:text-stone-400'}`}
          onClick={() => setActiveTab('openOrders')}
        >
          Open Orders
        </button>
        <button
          className={`cursor-pointer px-4 py-2 ${activeTab === 'fillOrders' ? 'text-white border-b-2 border-blue-500' : 'text-stone-300 hover:text-stone-400'}`}
          onClick={() => setActiveTab('fillOrders')}
        >
          Filled Orders
        </button>
        <button
          className={`cursor-pointer px-4 py-2 ${activeTab === 'orderHistory' ? 'text-white border-b-2 border-blue-500' : 'text-stone-300 hover:text-stone-400'}`}
          onClick={() => setActiveTab('orderHistory')}
        >
          Order History
        </button>
        <button
          className={`cursor-pointer px-4 py-2 ${activeTab === 'positionHistory' ? 'text-white border-b-2 border-blue-500' : 'text-stone-300 hover:text-stone-400'}`}
      ...
      {/* Content Area */}
      {/* The `overflow-auto` class is crucial here for enabling scrolling. */}
      <div className='mt-4 p-4 mr-6 bg-[#272933] rounded-2xl h-80 overflow-y-hidden overflow-x-auto'>
        {isLoadingData ? (
          ...
          <>
            ...
            {activeTab === 'openOrders' && (
              <div>
                ...
                {openOrders.length > 0 ? (
                  <div className="min-w-full inline-block"> {/* Ensure table content isn't restricted by parent's width */}
                    <table className="min-w-full divide-y divide-gray-700"> {/* min-w-full for the table is good */}
